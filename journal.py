...
def count_trades_session(session: str) -> int:
    with _conn() as conn:
        row = conn.execute(
            "SELECT COUNT(*) AS c FROM trades WHERE date = ? AND session = ?",
            (_today_str(), session),
        ).fetchone()
        return row["c"]

def count_losing_trades_session(session: str) -> int:
    with _conn() as conn:
        row = conn.execute(
            "SELECT COUNT(*) AS c FROM trades WHERE date = ? AND session = ? AND result = 'LOSS'",
            (_today_str(), session),
        ).fetchone()
        return row["c"]
...
def add_trade(symbol, side, entry, sl, tp, qty, risk_pct, leverage, rr, mode,
              order_id="", session="", confidence=None) -> dict:
