...
class BybitEngine:
    ...
    def place_order(
        ...
        _TICK_SIZES = {"BTCUSDT": 0.10, "ETHUSDT": 0.01, "SOLUSDT": 0.010, "LINKUSDT": 0.001, "HYPEUSDT": 0.010, "ONDOUSDT": 0.0001, "WLDUSDT": 0.0001, "BNBUSDT": 0.10, "XRPUSDT": 0.0001, "DOTUSDT": 0.0001, "ATOMUSDT": 0.0010, "UNIUSDT": 0.001, "CRVUSDT": 0.0001}
        _tick = _TICK_SIZES.get(symbol, 0.01)
        if sl:
            sl = round(round(sl / _tick) * _tick, 8)
        if tp:
            tp = round(round(tp / _tick) * _tick, 8)
        if not self._client:
            return self._not_ready()
        try:
            try:
    ...
    def close_position(self, symbol: str) -> dict:
        ...
        try:
            ...
            if response.get("retCode") == 0:
                ...
                pnl_data = self.get_closed_pnl(symbol, limit=10)
                closed_pnl = 0.0
                if pnl_data.get("ok") and pnl_data.get("records"):
                    match = next(
                        (r for r in pnl_data["records"] if r.get("orderId") == order_id),
                        None,
                    )
                    if match:
                        closed_pnl = match["closed_pnl"]
                    else:
                        match = next((r for r in pnl_data["records"] if r.get("orderId") == order_id), None)
                        closed_pnl = match["closed_pnl"] if match else pnl_data["records"][0]["closed_pnl"]
