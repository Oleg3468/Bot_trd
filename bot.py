...
from risk_manager import can_open_trade, build_trade_plan, MARGIN_PER_TRADE, LEVERAGE, MAX_TRADES_DAY, MAX_LOSSES_DAY, SESSION_LIMITS
...
async def handle_auto(update: Update, parts: list):
    ...
    if cmd2 in ("вкл","on","включить","start"):
        ...
        sess = get_current_session()
        lim = SESSION_LIMITS.get(sess['name'], {"max_trades": MAX_TRADES_DAY, "max_losses": MAX_LOSSES_DAY})
        await update.message.reply_text(f"🤖 *Автоторговля включена!*\nСканирую каждые 15 минут.\nСессия: {sess['emoji']} {sess['name']}\nЛимиты: {lim['max_trades']} сделок/день | {lim['max_losses']} убытка/день", parse_mode="Markdown")
