...
class AutoTrader:
    SYMBOLS = ["BTCUSDT", "ETHUSDT", "SOLUSDT", "LINKUSDT", "HYPEUSDT", "ONDOUSDT", "WLDUSDT", "BNBUSDT", "XRPUSDT", "DOTUSDT", "ATOMUSDT", "UNIUSDT", "CRVUSDT"]
    ...
    async def execute_signal(self, signal: SMCSignal) -> dict:
        ...
        def _execute_sync():
            ...
            if not result["ok"]:
                logger.error(f"place_order failed ({signal.symbol} {signal.side} qty={r.qty}): {result.get('msg')}")
            if result["ok"]:
