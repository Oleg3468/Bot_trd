...
AUDIT_STAMP="/tmp/bybit_bot_last_audit"
run_audit_if_needed() {
    TODAY=$(date +%Y-%m-%d)
    LAST=$(cat "$AUDIT_STAMP" 2>/dev/null || echo "")
    if [ "$TODAY" != "$LAST" ]; then
        echo "$(date): запускаю ежедневный аудит журнала"
        python3 journal_audit.py >> audit.log 2>&1
        python3 update_clean_dataset.py >> audit.log 2>&1
        echo "$TODAY" > "$AUDIT_STAMP"
    fi
}
